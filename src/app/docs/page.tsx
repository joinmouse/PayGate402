...
export default function DocsPage() {
  ...
  return (
    <>
      {/* Global shared header — same as all other pages */}
      <Header />

      {/* Body below header: sidebar + content */}
      <div style={{ display: "flex", paddingTop: 56, minHeight: "100vh" }}>

        {/* ── Sidebar ── */}
        <aside className="hidden lg:flex" style={{ width: 240, flexShrink: 0, flexDirection: "column", position: "fixed", top: 56, bottom: 0, left: 0, borderRight: "1px solid var(--border)", background: "var(--bg)", padding: "24px 16px", overflowY: "auto" }}>
        ...
        {/* ── Content ── */}
        <div className="lg:ml-[240px]" style={{ flex: 1, padding: "32px clamp(24px, 5vw, 64px) 80px" }}>
          <div style={{ maxWidth: 720 }}>
            ...
            <Step num={2} id="middleware" title="Add Payment Middleware">
              <p className="text-body" style={{ marginBottom: 4 }}>The middleware intercepts requests to your paid endpoints. No valid payment → automatic <code style={{ fontSize: 12, background: "var(--bg-code)", padding: "2px 6px", borderRadius: 4 }}>402</code> response with pricing info.</p>
  ...
  "POST /api/ai-summary": {
    ...
    network: "base-sepolia"
            ...
            <Step num={3} id="route" title="Write Your API Route">
              <p className="text-body" style={{ marginBottom: 4 }}>Your route stays exactly the same — middleware handles payment. If the request reaches your handler, payment is already verified.</p>
  ...
  return NextResponse.json({
    ...
    condition: "Sunny"
            ...
            <Step num={4} id="deploy" title="Deploy to Vercel">
              <p className="text-body" style={{ marginBottom: 4 }}>Set three environment variables from <a href="https://portal.cdp.coinbase.com" target="_blank" rel="noopener noreferrer" style={{ color: "var(--accent-text)", textDecoration: "none" }}>Coinbase Developer Platform</a>, then deploy.</p>
...
vercel deploy --prod`}</Code>
            ...
            <Step num={5} id="client" title="Client / Agent Integration">
              <p className="text-body" style={{ marginBottom: 4 }}><code style={{ fontSize: 12, background: "var(--bg-code)", padding: "2px 6px", borderRadius: 4 }}>x402Fetch</code> is a drop-in <code style={{ fontSize: 12, background: "var(--bg-code)", padding: "2px 6px", borderRadius: 4 }}>fetch()</code> replacement that auto-handles 402 → pay → retry.</p>
